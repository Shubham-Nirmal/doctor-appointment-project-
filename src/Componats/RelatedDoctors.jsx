...
const RelatedDoctors = ({speciality, docId}) => {
    ...
    return (
        <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
           ...
           <p className='sm:w-1/3 text-center text-sm'>Other doctors in the same speciality.</p>
           <div className='w-full grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
                {relDoc.slice(0,5).map((item, index)=> (
                <div onClick={() => {navigate(`/appointment/${item._id}`);scrollTo(0,0)}} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500 h-full' key={index}>
